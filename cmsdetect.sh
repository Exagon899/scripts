...
# Stage 1b: the literal word "Version" immediately labelling a number, in a
# label-style position (right after a tag boundary, sentence start, or line
# start) - e.g. "<h2>Version 1.4</h2>" or "Version: 2.2.8". This is independent
# of Stage 1's ?ver=/asset-trigger-word requirement, so it catches CMS that
# print their version as plain visible text rather than in an asset query
# string (e.g. FUEL CMS's "Welcome to Fuel CMS / Version 1.4" homepage banner).
# Deliberately requires the tag/sentence-boundary anchor so it does NOT fire on
# ordinary prose mentioning "version" mid-sentence ("uses version 2 of the API").
VERSION_VALUE_OR_MAJOR_RE = r'\d{1,4}(?:\.\d{1,4}){1,3}|\d{1,4}'
STAGE1B_RE = re.compile(
    r'(?:^|>|\.\s)\s*[Vv]ersion[\s:=]{0,3}(' + VERSION_VALUE_OR_MAJOR_RE + r')\b'
)
...
STAGE2_NEGATIVE_KEYWORDS = [
    ...
    "dtd html 4.01", "dtd xhtml 1.0", "dtd xhtml 1.1", "dtd html ",
    # Apache server version in directory-listing/error-page footers (mod_autoindex,
    # default error pages) - "Apache/2.4.18 (Ubuntu) Server at ..."
    "apache/",
    # jQuery core/migrate library versions bundled with virtually every CMS -
    # extremely common false positive, looks identical in shape to a CMS version
    "jquery.min.js?ver=", "migrate.min.js?ver=",
    # Inline SVG attributes (width/height/viewBox/enable-background) - SVG logos
    # embedded directly in HTML produce large numbers of dotted-decimal coordinate
    # values that look exactly like version numbers
    'width="', "width='", 'height="', "height='",
    'viewbox="', "viewbox='", 'enable-background="', "enable-background='",
    # Atom feed title version string - same pattern as RSS, fixed spec value
    'title="atom ', "title='atom ",
    # Joomla's bundled custom-element UI components (Cassiopeia template) version
    # their own assets separately from Joomla core - not the Joomla version itself
    "joomla-custom-elements/", "joomla-alert.min.css",
]
...
def detect_versions(text):
    """Returns {"stage": 0/1/1.5/2, "top": [{"version","count","line"}...], "ambiguous": bool}"""
    if not text:
    ...
    hits = [
        ...
        if not any(lib in path.lower() for lib in KNOWN_LIBRARY_PATHS)
    ]
    stage = 1
    if not hits:
        hits = [(m.group(1), m.start()) for m in STAGE1B_RE.finditer(text)]
        stage = 1.5
    if not hits:
    ...
    # Soft cap instead of a hard top-3: show every distinct candidate up to a
    # generous limit. A well-tuned negative-keyword list (see above) should
    # keep the real candidate count small in practice; if it doesn't, that's
    # itself useful signal that the negative list needs another entry - so we
    # report the overflow honestly instead of silently truncating to 3.
    SOFT_CAP = 15
    top_values = distinct_sorted[:SOFT_CAP]
    hidden_count = max(0, len(distinct_sorted) - SOFT_CAP)
    top = [
        {"version": v, "count": counts[v], "line": get_line_for_offset(text, first_offset[v])}
        for v in top_values
    ]
    ambiguous = len(top_values) > 1
    return {"stage": stage, "top": top, "ambiguous": ambiguous, "hidden_count": hidden_count}
...
for r in results[:MAX_CMS_TO_VERSION_CHECK]:
    ...
    # Try fallback version_paths not just when the main page found NOTHING, but
    # also when it found only an ambiguous/noisy result (e.g. several Stage 2
    # candidates from feed specs, library assets, etc.) - a dedicated version-
    # disclosure file (README, CHANGELOG, manifest XML) is often more
    # authoritative than scraping the homepage. If the fallback paths don't
    # yield a clean answer either, the original main-page result (even if
    # ambiguous) is kept rather than being discarded - we never want to end up
    # with LESS information than what the main page already gave us.
    if detection["stage"] == 0 or detection.get("ambiguous"):
        ...
        for path in paths_to_try[:MAX_PATHS_TO_TRY]:
            ...
            if page_text:
                candidate = detect_versions(page_text)
                if candidate["stage"] != 0 and not candidate.get("ambiguous"):
                    # found a clean, unambiguous answer from a dedicated source - use it
                    detection = candidate
                    source = path
                    break
                elif candidate["stage"] != 0 and detection["stage"] == 0:
                    # main page had nothing at all - even an ambiguous fallback
                    # result is strictly better than nothing, so adopt it
                    detection = candidate
                    source = path
        else:
            # no break occurred - no clean unambiguous fallback found, restore
            # the original main-page result if it had at least something
            if original_detection["stage"] != 0 and detection["stage"] == 0:
...
for r in results:
    if r.get("name_only"):
        ...
        if not top:
            ...
            hidden = vd.get("hidden_count", 0)
            hidden_note = f" ({hidden} more hidden, consider extending the negative keyword list)" if hidden else ""
            ...
            for t in top:
                ...
                print(f"      {t['line']}")
            if hidden_note:
                print(f"    {CYAN}{hidden_note.strip()}{NC}")
        print()
        continue
    ...
    #   - 2+ different distinct values -> yellow, list all (up to the soft cap)
    #     with source line, plus an honest note if more were hidden beyond the cap
    vd = r.get("version_detection", {"stage": 0, "top": [], "ambiguous": False})
    top = vd.get("top", [])
